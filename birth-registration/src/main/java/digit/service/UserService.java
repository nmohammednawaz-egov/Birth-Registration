...
public class UserService {
    ...
    public void callUserService(BirthRegistrationRequest request){
        request.getBirthRegistrationApplications().forEach(application -> {
            if(!StringUtils.isEmpty(application.getFatherOfApplicant().getId()))
                enrichUser(application, request.getRequestInfo());
            else {
                User user = createFatherUser(application);
                application.getFatherOfApplicant().setId(upsertUser(user, request.getRequestInfo()));
            }
        });

        request.getBirthRegistrationApplications().forEach(application -> {
            if(!StringUtils.isEmpty(application.getMotherOfApplicant().getId()))
                enrichUser(application, request.getRequestInfo());
            else {
                User user = createMotherUser(application);
                application.getMotherOfApplicant().setId(upsertUser(user, request.getRequestInfo()));
            }
        });
    }

    private User createFatherUser(BirthRegistrationApplication application){
        FatherApplicant father = application.getFatherOfApplicant();
...
//        String tenantId = father.getTenantId();
    ...
    private User createMotherUser(BirthRegistrationApplication application){
        MotherApplicant mother = application.getMotherOfApplicant();
...
//        String tenantId = father.getTenantId();
    ...
    private void enrichUser(BirthRegistrationApplication application, RequestInfo requestInfo){
        String accountIdFather = application.getFatherOfApplicant().getId();
        String accountIdMother = application.getMotherOfApplicant().getId();
        ...
        else application.getFatherOfApplicant().setId(userDetailResponseFather.getUser().get(0).getUuid());

        if(userDetailResponseMother.getUser().isEmpty())
            throw new CustomException("INVALID_ACCOUNTID","No user exist for the given accountId");

        else application.getMotherOfApplicant().setId(userDetailResponseMother.getUser().get(0).getUuid());
